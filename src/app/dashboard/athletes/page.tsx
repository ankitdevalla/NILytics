          ...
          {athletes.length > 0 && (
            <button
              onClick={async () => {
                ...
                console.log(
                  `Debug: Attempting deletion of athlete ${athlete.id}`
                );
                ...
                try {
                  ...
                  const { data: athleteData, error: fetchError } =
                    await supabase
                      .from("athletes")
                      .select("*")
                      .eq("id", athlete.id)
                  ...
                  const { data: paymentData, error: paymentFetchError } =
                    await supabase
                      .from("payments")
                  ...
                  if (paymentFetchError) {
                    ...
                    alert(
                      `Error fetching payments: ${paymentFetchError.message}`
                    );
                    return;
                  }

                  console.log(
                    `Found ${paymentData.length} related payments:`,
                    paymentData
                  );
                  ...
                  if (paymentData.length > 0) {
                    ...
                    if (deletePaymentsError) {
                      ...
                      alert(
                        `Error deleting payments: ${deletePaymentsError.message}`
                      );
                      return;
                    }
                  ...
                  if (deleteAthleteError) {
                    ...
                    alert(
                      `Error deleting athlete: ${deleteAthleteError.message}`
                    );
                    return;
                  }
